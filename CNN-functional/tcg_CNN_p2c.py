...
#       - 04, Apr 23: implemented f1-score check point
#
...
#
# implement f1-score function here, using the binary y_true input, but
# real value y_pred between [0,1]. If y_pred > threshold, it will be
# classified as a label 1. By default, threshold = 0.5. Need further
# changes here.
#
import keras.backend as K
def get_f1_default(y_true, y_pred, threshold):
...
def get_f1_chanh(y_true, y_pred, threshold):
    return f1
#
...
for dense_layer in dense_layers:
    for layer_size in layer_sizes:
        for conv_layer in conv_layers:
            ...
            #
            # Either implement f1 score as an output metric as in here, or can use the
            # call callbacks as keras.callbacks.ModelCheckpoint(filepath=NAME, monitor='get_f1_chanh',
            #                                                   mode='max',save_best_only=True)
            #       
            callbacks=[keras.callbacks.ModelCheckpoint(NAME,save_best_only=True)]
            #model.compile(loss="binary_crossentropy",optimizer="adam",metrics=["accuracy"])
            model.compile(loss="binary_crossentropy",optimizer="adam",metrics=[get_f1_chanh])
